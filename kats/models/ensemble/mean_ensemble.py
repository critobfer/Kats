...
"""Mean ensembling method

Ensemble models with mean of individual models
Assume we have k base models, after we make forecasts with each individual
model, we take the mean from each time point as the final results
"""
...
class MeanEnsembleModel(ensemble.BaseEnsemble):
    """Mean ensemble model class
    ...
    def predict(self, steps: int, **kwargs) -> pd.DataFrame:
        """Predict method of mean ensemble model
        ...
        self.fcst = fcst_all.mean(axis=1) 
    ...
    def __str__(self) -> str:
        """Get default parameter search space for the mean ensemble model
